                                                 ...
                                                 const VolumeData & samples, u8 lodIndex = 0)
{
        ...
        if ((t & 0x00ff) == 0)
        {
                ...
                if (t == 0) {
                        return v1.cast<double>();
                }
                return v0.cast<double>();
        }
        else
        {
                ...
                return (v0.cast<double>() * t) * s + (v1.cast<double>() * u) * s;
        }
}
        ...
        regular_cell_cache * cache)
{
        ...
        for (int i = 0; i < 3; ++i)
        {
                if (min[i] ==    0) { near |= (1 << (i * 2 + 0)); /* Vertex close to negative face. */ }
                if (min[i] == last) { near |= (1 << (i * 2 + 1)); /* Vertex close to positive face. */ }
        }
        ...
        for (u8 i = 0; i < nv; ++i)
        {
                ...
                if ((t & 0x00ff) != 0)
                {
                        ...
                        if (!present || localVertexMapping[i] < 0)
                        {
                                ...
                                const Vector3d pi = interp(p0.cast<double>().eval(), p1.cast<double>().eval(), p0, p1, samples, lodIndex);
                ...
                else if(t == 0 && v1 == 7)
                {
                        ...
                        const Eigen::Vector3d pi = p0.cast<double>() * t0 + p1.cast<double>() * t1;
        ...
        transition_cell_cache * cache)
{
        ...
        for (int i = 0; i < 3; ++i)
        {
                if (origin[i] ==    0) { near |= (1 << (i * 2 + 0)); /* Vertex close to negative face. */ }
                if (origin[i] == last) { near |= (1 << (i * 2 + 1)); /* Vertex close to positive face. */ }
        }
        ...
        for (long i = 0; i < nv; ++i)
        {
                ...
                if ((t & 0x00ff) != 0)
                {
                        ...
                        else
                        {
                                ...
                                const Vector3d p0 = pos[v0].cast<double>();
                                const Vector3d p1 = pos[v1].cast<double>();
